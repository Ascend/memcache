...
使用memfabric_hybrid相关接口时，需要include相关头文件(在${MEMFABRIC_HYBRID_HOME_PATH}/aarch64-linux/include/路径下)，并且在**链接时需要添加对应so**(在${MEMFABRIC_HYBRID_HOME_PATH}/aarch64-linux/lib64路径下)依赖
...
#### Python接口
安装完成run包并source安装路径下的set_env.sh后，即可在python中通过import方式导入memfabric_hybrid的python包，然后调用python接口

python接口为c接口的封装，功能一致，具体介绍可以在python中使用help函数获取，参考如下
```python
import mf_smem      # 导入smem
help(mf_smem)       # 查看smem基础函数介绍

...
| DlDfcApi | 动态加载DFC库的接口封装 | src/memcache/csrc/under_api/ubs_io/dl_dfc_api.h/cpp |
| DFC库 | 分布式文件缓存库，处理SSD存储操作 | 外部依赖：libdfc_client.so |

### 2.3 数据流向

1. **写入路径**：应用 → MemCache API → 内存池 → (淘汰) → UBS_IO代理 → DFC库 → SSD
2. **读取路径**：应用 → MemCache API → 内存池(命中) → 返回
   - 未命中：应用 → MemCache API → 内存池 → UBS_IO代理 → DFC库 → SSD → 内存池 → 返回
...
- **DFC库**：需要安装Distributed File Cache (DFC)库
  - 安装方法：参考[UBS IO](https://gitcode.com/UBSCore/DistFileCache)
  - 库文件：libdfc_client.so
...
| 问题 | 可能原因 | 解决方案 |
|------|----------|----------|
| SSD写入失败 | DFC库未正确安装 | 检查DFC库安装情况，确保libdfc_client.so可被找到 |

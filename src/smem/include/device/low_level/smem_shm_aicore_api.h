 ...
 * @param smemId           [in] shm object id, default 0
 */
SMEM_INLINE_AICORE uint32_t smem_shm_get_global_rank(uint32_t smemId = 0);

/**
 * @brief Get rank size which is set by function smem_shm_create from host side
 * @param smemId           [in] shm object id, default 0
 */
SMEM_INLINE_AICORE uint32_t smem_shm_get_global_rank_size(uint32_t smemId = 0);

/**
 * @brief Get symmetric size which is set by function smem_shm_create from host side
 * @param smemId           [in] shm object id, default 0
 */
SMEM_INLINE_AICORE uint64_t smem_shm_get_symmetric_size(uint32_t smemId = 0);

/**
 * @brief Get user extra context addr (context is set by function smem_shm_set_extra_context from host side)
 * @param smemId           [in] shm object id, default 0
 */
SMEM_INLINE_AICORE __gm__ void* smem_shm_get_extra_context_addr(uint32_t smemId = 0);

/**
 * @brief Get user extra context size (context is set by function smem_shm_set_extra_context from host side)
 * @param smemId           [in] shm object id, default 0
 */
SMEM_INLINE_AICORE uint32_t smem_shm_get_extra_context_size(uint32_t smemId = 0);
